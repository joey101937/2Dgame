 ...
 * @author Joseph
 */
public class BasicBoat extends Unit{

    public BasicBoat(int x, int y, int team) {
        super(x, y, team);
        this.name = "Boat";
        this.desc = "Core Navel Vessel";
        this.plane = Plane.sea;
        this.health = 100;
        this.maxHealth = 100;
        ...
        for (int i = 0; i < 8; i++) {
            this.spriteIcon[i] = new File(Main.getDir() + Main.assets +"PrimitiveBoat.png");
        }
       for (int i = 0; i < 8; i++) {
            this.MFspriteIcon[i] = new File(Main.getDir() + Main.assets + "PrimitiveBoat.png");
        }
       this.loadSprites();
       this.loadMFSprites();
       this.mCost = 100;
